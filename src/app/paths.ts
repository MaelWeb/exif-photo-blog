...
export const doesPathOfferSort = (pathname: string) =>
  pathname === PATH_ROOT || pathname.startsWith(PATH_GRID) || pathname.startsWith(PATH_FEED);
...
export const PATH_API_QINIU_UPLOAD = `${PATH_API_STORAGE}/qiniu/upload`;
...
export const pathForAdminUploadUrl = (url: string, title?: string) =>
  `${PATH_ADMIN_UPLOADS}/${encodeURIComponent(url)}${
    title ? `?${PARAM_UPLOAD_TITLE}=${encodeURIComponent(title)}` : ''
  }`;

export const pathForAdminPhotoEdit = (photo: PhotoOrPhotoId) => `${PATH_ADMIN_PHOTOS}/${getPhotoId(photo)}/${EDIT}`;
...
export const pathForPhoto = ({ photo, recent, year, camera, lens, tag, film, focal, recipe }: PhotoPathParams) => {
...
export const pathForYear = (year: string) => `${PREFIX_YEAR}/${year}`;

// Image paths
const pathForImage = (path: string) => `${path}/${IMAGE}`;

export const pathForPhotoImage = (photo: PhotoOrPhotoId) => pathForImage(pathForPhoto({ photo }));

export const pathForCameraImage = (camera: Camera) => pathForImage(pathForCamera(camera));

export const pathForLensImage = (lens: Lens) => pathForImage(pathForLens(lens));

export const pathForTagImage = (tag: string) => pathForImage(pathForTag(tag));

export const pathForRecipeImage = (recipe: string) => pathForImage(pathForRecipe(recipe));

export const pathForFilmImage = (film: string) => pathForImage(pathForFilm(film));

export const pathForFocalLengthImage = (focal: number) => pathForImage(pathForFocalLength(focal));

export const pathForYearImage = (year: string) => pathForImage(pathForYear(year));

export const pathForRecentsImage = () => pathForImage(PREFIX_RECENTS);

// Absolute paths
export const ABSOLUTE_PATH_FOR_FEED_JSON = `${getBaseUrl()}${PATH_FEED_JSON}`;

export const ABSOLUTE_PATH_FOR_RSS_XML = `${getBaseUrl()}${PATH_RSS_XML}`;
...
export const absolutePathForLens = (lens: Lens, share?: boolean) => `${getBaseUrl(share)}${pathForLens(lens)}`;

export const absolutePathForTag = (tag: string, share?: boolean) => `${getBaseUrl(share)}${pathForTag(tag)}`;

export const absolutePathForRecipe = (recipe: string, share?: boolean) =>
  `${getBaseUrl(share)}${pathForRecipe(recipe)}`;

export const absolutePathForFilm = (film: string, share?: boolean) => `${getBaseUrl(share)}${pathForFilm(film)}`;

export const absolutePathForFocalLength = (focal: number, share?: boolean) =>
  `${getBaseUrl(share)}${pathForFocalLength(focal)}`;

export const absolutePathForYear = (year: string, share?: boolean) => `${getBaseUrl(share)}${pathForYear(year)}`;

export const absolutePathForRecents = (share?: boolean) => `${getBaseUrl(share)}${PREFIX_RECENTS}`;

export const absolutePathForPhotoImage = (photo: PhotoOrPhotoId) => `${getBaseUrl()}${pathForPhotoImage(photo)}`;

export const absolutePathForCameraImage = (camera: Camera) => `${getBaseUrl()}${pathForCameraImage(camera)}`;

export const absolutePathForLensImage = (lens: Lens) => `${getBaseUrl()}${pathForLensImage(lens)}`;

export const absolutePathForTagImage = (tag: string) => `${getBaseUrl()}${pathForTagImage(tag)}`;

export const absolutePathForRecipeImage = (recipe: string) => `${getBaseUrl()}${pathForRecipeImage(recipe)}`;

export const absolutePathForFilmImage = (film: string) => `${getBaseUrl()}${pathForFilmImage(film)}`;

export const absolutePathForFocalLengthImage = (focal: number) => `${getBaseUrl()}${pathForFocalLengthImage(focal)}`;

export const absolutePathForYearImage = (year: string, share?: boolean) =>
  `${getBaseUrl(share)}${pathForYearImage(year)}`;

export const absolutePathForRecentsImage = (share?: boolean) => `${getBaseUrl(share)}${pathForRecentsImage()}`;

// p/[photoId]
export const isPathPhoto = (pathname = '') => new RegExp(`^${PREFIX_PHOTO}/[^/]+/?$`).test(pathname);

// recents
export const isPathRecents = (pathname = '') => new RegExp(`^${PREFIX_RECENTS}/?$`).test(pathname);

// recents/[photoId]
export const isPathRecentsPhoto = (pathname = '') => new RegExp(`^${PREFIX_RECENTS}/[^/]+/?$`).test(pathname);

// year/[year]
export const isPathYear = (pathname = '') => new RegExp(`^${PREFIX_YEAR}/[^/]+/?$`).test(pathname);

// year/[year]/[photoId]
export const isPathYearPhoto = (pathname = '') => new RegExp(`^${PREFIX_YEAR}/[^/]+/[^/]+/?$`).test(pathname);

// shot-on/[make]/[model]
export const isPathCamera = (pathname = '') => new RegExp(`^${PREFIX_CAMERA}/[^/]+/[^/]+/?$`).test(pathname);

// shot-on/[make]/[model]/[photoId]
export const isPathCameraPhoto = (pathname = '') => new RegExp(`^${PREFIX_CAMERA}/[^/]+/[^/]+/[^/]+/?$`).test(pathname);

// lens/[make]/[model]
export const isPathLens = (pathname = '') => new RegExp(`^${PREFIX_LENS}/[^/]+/[^/]+/?$`).test(pathname);

// lens/[make]/[model]/[photoId]
export const isPathLensPhoto = (pathname = '') => new RegExp(`^${PREFIX_LENS}/[^/]+/[^/]+/[^/]+/?$`).test(pathname);

// tag/[tag]
export const isPathTag = (pathname = '') => new RegExp(`^${PREFIX_TAG}/[^/]+/?$`).test(pathname);

// tag/[tag]/[photoId]
export const isPathTagPhoto = (pathname = '') => new RegExp(`^${PREFIX_TAG}/[^/]+/[^/]+/?$`).test(pathname);

// recipe/[recipe]
export const isPathRecipe = (pathname = '') => new RegExp(`^${PREFIX_RECIPE}/[^/]+/?$`).test(pathname);

// recipe/[recipe]/[photoId]
export const isPathRecipePhoto = (pathname = '') => new RegExp(`^${PREFIX_RECIPE}/[^/]+/[^/]+/?$`).test(pathname);

// film/[film]
export const isPathFilm = (pathname = '') => new RegExp(`^${PREFIX_FILM}/[^/]+/?$`).test(pathname);

// film/[film]/[photoId]
export const isPathFilmPhoto = (pathname = '') => new RegExp(`^${PREFIX_FILM}/[^/]+/[^/]+/?$`).test(pathname);

// focal/[focal]
export const isPathFocalLength = (pathname = '') => new RegExp(`^${PREFIX_FOCAL_LENGTH}/[^/]+/?$`).test(pathname);
...
export const isPathTopLevelAdmin = (pathname?: string) => PATHS_ADMIN.some((path) => path === pathname);

export const isPathAdminPhotos = (pathname?: string) => checkPathPrefix(pathname, PATH_ADMIN_PHOTOS);

export const isPathAdminInsights = (pathname?: string) => checkPathPrefix(pathname, PATH_ADMIN_INSIGHTS);

export const isPathAdminConfiguration = (pathname?: string) => checkPathPrefix(pathname, PATH_ADMIN_CONFIGURATION);
...
export const getPathComponents = (
  ...
  const photoIdFromCamera = pathname.match(new RegExp(`^${PREFIX_CAMERA}/[^/]+/[^/]+/([^/]+)`))?.[1];
  ...
  const photoIdFromFocalLength = pathname.match(new RegExp(`^${PREFIX_FOCAL_LENGTH}/[0-9]+mm/([^/]+)`))?.[1];
  const photoIdFromYear = pathname.match(new RegExp(`^${PREFIX_YEAR}/[^/]+/([^/]+)`))?.[1];
  const photoIdFromRecents = pathname.match(new RegExp(`^${PREFIX_RECENTS}/([^/]+)`))?.[1];
  ...
  const year = pathname.match(new RegExp(`^${PREFIX_YEAR}/([^/]+)`))?.[1];
...
export const getEscapePath = (pathname?: string) => {
  const { photoId, recent, year, camera, lens, tag, recipe, film, focal } = getPathComponents(pathname);
